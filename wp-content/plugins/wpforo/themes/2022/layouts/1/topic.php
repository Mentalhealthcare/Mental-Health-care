		...
		<div class="topic-wrap <?php wpforo_unread($topic['topicid'], 'topic'); ?>">
          ...
          <div class="wpforo-last-posts-<?php echo intval($topic['topicid']) ?>" style="display: <?php echo ( $post_toglle == 1 ? 'block' : 'none' ); ?>">
              <div class="wpforo-last-posts-list">
                <ul>
                    <li>
                        <div class="wpforo-last-post-title"><i class="fas fa-feather-alt wpfsx wpfcl-0"></i> &nbsp; <a href="<?php echo esc_url($first_post['url']) ?>"><?php echo esc_html( wpforo_text($first_post['body'], wpforo_setting( 'topics', 'layout_extended_intro_posts_length' ), false)) ?></a></div>
					...
					<?php if(!empty($posts) && is_array($posts)) : ?>
                        ...
                        <?php if(intval($topic['posts']) > ($intro_posts+1)): ?>
                            <li class="wpforo-last-post-more"><a href="<?php echo esc_url($topic_url) ?>"><?php wpforo_phrase('view all posts', true, 'lower');  ?> <i class="fas fa-angle-right" aria-hidden="true"></i></a></li>
                        <?php endif ?>
					<?php endif ?>
                </ul>
